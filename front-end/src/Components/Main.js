...
export const Main = () => {
  ...
  useEffect(() => { // Checks to see what role we have when the user logins
    axios.get("http://localhost:2000/api/getrole").then((res) => {
        setRole(res.data.role)
        setLoading(false);
    }).catch((err) => { // If we have an error where we try to access the page without logging in
        console.log(err);
